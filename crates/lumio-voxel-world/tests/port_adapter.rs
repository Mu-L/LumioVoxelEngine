...
use lumio_voxel_contracts::{BoundedBuffer, sha256};
...
use lumio_voxel_domain::revision::{RevisionStamp, WorldRevision};
...
use lumio_voxel_ops::query::{BlockReadWorld, VoxelQueryRequest};
...
use lumio_voxel_world::port::{
    MutationStatus, OwnedResultBuffer, PORT_METHODS, PORT_RUST_TYPE, PORT_SCHEMA,
    VoxelWorldPortAdapter, map_internal_error,
};
...
fn bindings_and_schema_ids_intern_voxel_world_port() {
    ...
    assert_eq!(interned_schema, "voxel-world-port");
    assert_eq!(interned_binding, "VoxelWorldPort");
...
fn adapter_query_matches_direct_world_stamp() {
    ...
    let via = VoxelWorldPortAdapter::new(&mut world)
...
fn adapter_mutation_matches_direct_receipt_txn() {
    ...
    let mut adapter = VoxelWorldPortAdapter::new(&mut via_world);
...
fn adapter_capture_restore_and_durability_ack_are_callable() {
    ...
    VoxelWorldPortAdapter::new(&mut world)
...
fn stale_generation_handle_maps_to_stale_epoch() {
    ...
    let err = VoxelWorldPortAdapter::new(&mut world)
...
fn generated_port_exposes_all_frozen_methods() {
    ...
    let mut world = VoxelWorldPortAdapter::create_world(
...
fn adapter_admit_returns_admitted_lifecycle() {
    ...
    let admitted = VoxelWorldPortAdapter::new(&mut world)
        .admit(cmd)
